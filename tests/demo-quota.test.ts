...
const vercelIntegrationEnv: NodeJS.ProcessEnv = {
  UPSTASH_REDIS_REST_KV_REST_API_URL: "https://vercel-example.upstash.io",
  UPSTASH_REDIS_REST_KV_REST_API_TOKEN:
    "vercel-test-token-that-never-leaves-the-test",
};
...
test("accepts credentials generated by the Vercel Upstash integration", async () => {
  const redis = new CounterRedis();
  const result = await checkPublicDemoQuota("203.0.113.8", {
    env: vercelIntegrationEnv,
    redis,
    now: new Date("2026-08-09T12:00:00.000Z"),
  });
...
test("prefers the canonical credential pair when both conventions exist", async () => {
  const canonicalRedis = new CounterRedis();
  const combinedRedis = new CounterRedis();
  const options = {
    now: new Date("2026-08-09T12:00:00.000Z"),
  };
  ...
  await checkPublicDemoQuota("203.0.113.8", {
    ...options,
    env: { ...vercelIntegrationEnv, ...baseEnv },
    redis: combinedRedis,
  });

  assert.equal(canonicalRedis.calls[0].keys[1], combinedRedis.calls[0].keys[1]);
});
