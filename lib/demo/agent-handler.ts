/** Agent 请求处理器。独立于 Route 文件以便执行拒绝路径测试。 */
...
interface AgentRouteDependencies {
  checkQuota: typeof checkPublicDemoQuota;
  streamGraph: (
    initialState: Partial<GatherState>
  ) => Promise<AsyncIterable<GatherState>>;
}

const defaultDependencies: AgentRouteDependencies = {
  checkQuota: checkPublicDemoQuota,
  streamGraph: async (initialState) => {
    const stream = await graph.stream(initialState, {
      streamMode: "values",
    });
    return stream as AsyncIterable<GatherState>;
  },
};

export async function handleAgentRequest(
  request: NextRequest,
  dependencies: AgentRouteDependencies = defaultDependencies
) {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          const eventStream = await dependencies.streamGraph(initialState);
